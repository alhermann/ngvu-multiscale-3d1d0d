...
public:
    ...
    BoundingBoxTree(const GridView& leafGridView, int verbose = 1)
    {
      if( leafGridView.size(0)>0) 
        build(leafGridView,verbose); 
    }

    //! Build up bounding box tree for a grid with leafGridView
    void build(const GridView& leafGridView, int verbose)
    {
        // clear data if any
        clear_();
