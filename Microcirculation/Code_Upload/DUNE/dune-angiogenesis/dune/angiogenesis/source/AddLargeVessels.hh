...
auto addLargeVessels(const GV1D& gv1d, const GV3D gv3d, Vector& R, Vector& G, Vector& mu, BlockVector& pO2, Vector& u,
                     RF rho_bl, RF gamma, RF mean, RF std_dev, RF lambda_g, Dune::FieldVector<RF, 3> O,
                     Dune::FieldVector<RF, 3> L, std::vector<Subdomain>& subdomains, int NSubdomain,
                     std::vector<RF>& avg_local_po2, RF threshold_LV, RF threshold_PO2) {
    ...
    for (const auto& element : elements(grid_copy->leafGridView())) {
        ...
        for (const auto& intersection : intersections(grid_copy->leafGridView(), element)) {
            if (intersection.boundary()) {
                ...
                if (!is_colliding && (subDomain1 != subDomain2) && R_p > threshold_LV) {
                ...
                if (!is_colliding && (avg_local_po2[subDomain1] < threshold_PO2 * 133.32239) && R_p > threshold_LV) {
                ...
                if (bifurcation) {
                    // RF R_2 = std::pow( std::pow(R_p, gamma) - std::pow(R_1, gamma), 1.0/gamma );
    ...
    if (counter_new_vessels > 0) {
        ...
        if (!newElementGenerated) {
            DUNE_THROW(Dune::Exception, "grid.grow() did not insert any new element");
        }
...
template <typename GV1D, typename Vector, typename GV3D>
auto extractNetworkInElement(const GV1D& gv1d, Vector& R, Vector& G, Vector& mu, Vector& u, const GV3D& gv3d) {
    typedef double RF;
    ...
    std::vector<Dune::FieldVector<RF, dimworld> > matrixOfVertices;
    std::vector<int> matrixOfConnections;
    std::vector<RF> R_red, G_red, mu_red, u_red;

    if (gv3d.size(0) != 1) {
        DUNE_THROW(Dune::Exception, "The 3D mesh has " << gv3d.size(0) << " elements instead of 1!");
    }

    BoundingBoxTree<GV3D> tree(gv3d, 0);
    typedef typename GV3D::template Codim<0>::Entity Element;
    IndexToElementMap<GV3D> indexToElementMap(gv3d);
    indexToElementMap.resize(gv3d.size(0));
    for (const auto& element : elements(gv3d)) {
    ...
    Element element3d = indexToElementMap.entity(0);
    // Determine corners of the hexahedral element
    Dune::FieldVector<RF, dimworld> corner0 = element3d.geometry().corner(0);
    Dune::FieldVector<RF, dimworld> corner7 = element3d.geometry().corner(7);

    // Generate new network
    for (const auto& element1d : elements(gv1d)) {
        int elementNumber = gv1d.indexSet().index(element1d);

        Dune::FieldVector<RF, dimworld> vertex1 = element1d.geometry().corner(0);
        Dune::FieldVector<RF, dimworld> vertex2 = element1d.geometry().corner(1);
        std::vector<unsigned int> searchingVertex1 = tree.computeEntityCollisions(vertex1);
        std::vector<unsigned int> searchingVertex2 = tree.computeEntityCollisions(vertex2);
        if (searchingVertex1.size() == 0 && searchingVertex2.size() == 0) continue;

        // We use the method collide of GeometryCollision.hh to derive the correct intersection of the segment with the
        // element
        auto geometryA = element3d.geometry();
        auto geometryB = element1d.geometry();
        using CollisionType = GeometryCollision<decltype(geometryA), decltype(geometryB)>;
        std::vector<Dune::FieldVector<RF, dimworld> > intersection;
        CollisionType::collide(geometryA, geometryB, intersection);

        if (intersection.size() == 0) continue;

        std::pair<bool, int> intersection0present = isVertexAlreadyPresent(matrixOfVertices, intersection[0]);
        std::pair<bool, int> intersection1present = isVertexAlreadyPresent(matrixOfVertices, intersection[1]);
        if (!intersection0present.first) {
            factory.insertVertex(intersection[0]);
            matrixOfVertices.push_back(intersection[0]);
            matrixOfConnections.push_back(matrixOfVertices.size() - 1);
        } else
            matrixOfConnections.push_back(intersection0present.second);

        if (!intersection1present.first) {
            factory.insertVertex(intersection[1]);
            matrixOfVertices.push_back(intersection[1]);
            matrixOfConnections.push_back(matrixOfVertices.size() - 1);
        } else
            matrixOfConnections.push_back(intersection1present.second);

        R_red.push_back(R[elementNumber]);
        G_red.push_back(G[elementNumber]);
        mu_red.push_back(mu[elementNumber]);

        // To simplify the computation, the pressure value is the one taken at the exact node 0 or 1, even if the
        // segment has been cut off by the collision geometry
        u_red.push_back(u[gv1d.indexSet().subIndex(element1d, 0, dimnetwork)]);
        u_red.push_back(u[gv1d.indexSet().subIndex(element1d, 1, dimnetwork)]);
    }

    // Save the corresponding data for the vessel of the arteries
    R.resize(R_red.size());
    G.resize(G_red.size());
    mu.resize(mu_red.size());
    for (int k = 0; k < R.N(); k++) {
        R[k] = R_red[k];
        G[k] = G_red[k];
        mu[k] = mu_red[k];
    ...
    u.resize(matrixOfVertices.size());
    u = 0.0;

    int i = 0;
    while (i < matrixOfConnections.size()) {
        std::vector<unsigned int> cornerIDs(2);
        cornerIDs[0] = matrixOfConnections[i];
        cornerIDs[1] = matrixOfConnections[i + 1];
        factory.insertElement(type, cornerIDs);

        // Save pressure values
        u[cornerIDs[0]] = u_red[i];
        u[cornerIDs[1]] = u_red[i + 1];
        i += 2;
    }

    // The element does not contain any part of the network. We have to return a network anyway
    if (matrixOfVertices.size() == 0) {
        Dune::FieldVector<RF, dimworld> vertex0(-1.0), vertex1(0.0);
    ...
    auto extractedNetwork = factory.createGrid();
    return extractedNetwork->leafGridView();
}
